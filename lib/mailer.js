...
const BRAND_LOGO_URL = process.env.BRAND_LOGO_URL || 'https://netrik-shop.vercel.app/brand/original/netrikshop%20update%20logo.png';
...
function toText(payload) {
  ...
  return [
    `Dear ${ownerName || 'Restaurant Partner'},`,
    '',
    `Welcome to Netrik Shop. I am thrilled to personally welcome you to our platform. Your premium workspace for ${restaurantName} has been meticulously configured and is now fully active.`,
    '',
    `We built Netrik Shop with a single vision: to provide restaurant owners with the most advanced, seamless technology so you can focus on what you do best—delivering exceptional culinary experiences.`,
    '',
    `Below are your official system credentials and restaurant details. Please keep this information secure.`,
    '',
    `Best regards,`,
    `The CEO, Netrik Shop`,
    '',
...
function toHtml(payload) {
  ...
  return `
    <div style="background-color:#f3f4f6;padding:40px 20px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#374151;line-height:1.6;">
      <div style="max-width:600px;margin:0 auto;background-color:#fdfdfa;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden;box-shadow:0 4px 6px -1px rgba(0, 0, 0, 0.05);">
        
        <!-- Header -->
        <div style="background-color:#e6ece9;padding:30px;text-align:center;border-bottom:1px solid #d1dbd5;">
          <img src="${BRAND_LOGO_URL}" alt="Netrik Shop" style="height:42px;width:auto;display:block;margin:0 auto 8px;" />
          <div style="color:#6b7280;font-size:12px;letter-spacing:2px;text-transform:uppercase;"> </div>
        </div>

        <!-- Body -->
        <div style="padding:40px 30px;">
          <h1 style="margin:0 0 16px 0;font-size:22px;font-weight:700;color:#111827;">Welcome to Netrik Shop</h1>
          <p style="margin:0 0 16px 0;font-size:16px;color:#4b5563;">Dear ${ownerName || 'Restaurant Partner'},</p>
          <p style="margin:0 0 16px 0;font-size:16px;color:#4b5563;line-height:1.7;">I am thrilled to personally welcome you to our platform. Your premium workspace for <strong>${restaurantName}</strong> has been meticulously configured and is now fully active.</p>
          <p style="margin:0 0 16px 0;font-size:16px;color:#4b5563;line-height:1.7;">We built Netrik Shop with a single vision: to provide restaurant owners with the most advanced, butter-smooth technology so you can focus on what you do best—delivering exceptional culinary experiences.</p>
          <p style="margin:0 0 30px 0;font-size:16px;color:#4b5563;line-height:1.7;">Below are your official system credentials and restaurant details. Please keep this information secure.<br><br>Warm regards,<br><strong>The CEO, Netrik Shop</strong></p>
          ...
          <div style="text-align:center;margin-bottom:40px;">
            <a href="https://netrik-shop.vercel.app/login" style="display:inline-block;background-color:#0f172a;color:#ffffff;text-decoration:none;font-weight:600;font-size:15px;padding:14px 28px;border-radius:6px;">Login to Dashboard</a>
          </div>
        ...
        <!-- Footer -->
        <div style="background-color:#f4f7f5;padding:24px 30px;text-align:center;border-top:1px solid #e5e7eb;">
...
export async function sendRestaurantOnboardingEmail(payload) {
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      to: payload.toEmail,
      subject: `Welcome to Netrik Shop: Exclusive Access & Credentials for ${payload.restaurantName}`,
      text: toText(payload),
      html: toHtml(payload),
    });
