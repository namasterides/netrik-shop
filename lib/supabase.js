...
// Public mapper: never includes passwords. Use this for any response a
// non-owner/non-central caller could see (menus, customer pages, etc.).
export const restaurantToApi = (r) => r ? ({
  ...
  upiId: r.upi_id || '',
  upiQrCode: r.upi_qr_code || '',
  createdAt: r.created_at,
  updatedAt: r.updated_at || null,
}) : null;

// Cred-bearing mapper: only for central admin views and the immediate response
// to restaurant creation (so the onboarding email can include credentials).
export const restaurantToApiWithCreds = (r) => r ? ({
  ...restaurantToApi(r),
...
export const menuToApi = (m) => m ? ({
  ...
  moodTags: Array.isArray(m.mood_tags) ? m.mood_tags : [],
  tasteTags: Array.isArray(m.taste_tags) ? m.taste_tags : [],
  dietaryTags: Array.isArray(m.dietary_tags) ? m.dietary_tags : [],
  promoted: !!m.promoted,
  promotedAt: m.promoted_at || null,
  promotionLabel: m.promotion_label || '',
}) : null;
...
export const orderToApi = (o) => o ? ({
  ...
  tipPercent: o.tip_percent || null,
  splitCount: o.split_count || null,
  totalWithTip: parseFloat(o.total_with_tip || 0),
